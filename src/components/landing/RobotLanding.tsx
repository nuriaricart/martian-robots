...
function RobotLanding(props: RobotLandingProps) {
  ...
  useEffect(() => {
    const validRobots: RobotType[] = robots
      .filter((robot) => checkWithinBounds(gridSize, robot.coord))
      .map((robot) => ({
        ...robot,
        finalDestination: {
          coord: { x: "", y: "" },
  ...
  const saveRobot = useCallback((robot: RobotType) => {
  ...
  const goBack = useCallback(() => {
    setIsRobotForm(false);
  }, [setIsRobotForm]);

  return (
    <Container className="sm:h-full flex-col justify-center gap-4">
      <div className="w-full flex gap-4 md:gap-8 items-center">
        <div className="md:w-2/5">
          <Button
            ...
            onClick={goBack}
            classNames={{
              buttonWrapperClassName: "self-start md:min-w-[150px]",
